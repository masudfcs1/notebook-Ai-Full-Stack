...
const benefits = [
  { icon: MessageSquareText, number: "01", title: "Every conversation, captured", text: "Import transcripts, upload notes, or capture meetings live. Every important detail stays searchable and organized.", color: "from-blue-500 to-cyan-400", visual: "transcript" },
  { icon: BrainCircuit, number: "02", title: "Context, not just a summary", text: "AI understands decisions, risks, and themes, then turns them into a concise brief your team can trust.", color: "from-violet-500 to-fuchsia-400", visual: "summary" },
  { icon: ListChecks, number: "03", title: "Momentum after the meeting", text: "Action items arrive with owners and due dates, ready to review, assign, and move into your workflow.", color: "from-emerald-500 to-teal-400", visual: "tasks" },
];
...
const fadeUp = { initial: { opacity: 0, y: 28 }, whileInView: { opacity: 1, y: 0 }, viewport: { once: true, margin: "-70px" } };
    ...
    <div className="landing-page landing-atmosphere min-h-screen overflow-x-hidden bg-[#f5f7ff] text-slate-950 selection:bg-blue-600 selection:text-white dark:bg-[#050817] dark:text-white">
      <header className="fixed inset-x-0 top-0 z-50 px-3 pt-3 sm:px-5 sm:pt-4">
        <div className="landing-nav-glass mx-auto flex h-[62px] max-w-[1180px] items-center justify-between rounded-[20px] px-4 sm:px-5 lg:px-6">
          ...
          <nav className="hidden items-center gap-1 rounded-full border border-slate-200/60 bg-white/45 p-1 lg:flex dark:border-white/10 dark:bg-white/[0.035]" aria-label="Primary navigation">{nav.map(([label, href]) => <a key={label} href={href} className="rounded-full px-4 py-2 text-[13px] font-medium text-slate-600 transition-colors hover:bg-white/80 hover:text-slate-950 dark:text-slate-300 dark:hover:bg-white/10 dark:hover:text-white">{label}</a>)}</nav>
          <div className="flex items-center gap-2">
            <button onClick={() => setTheme(theme === "dark" ? "light" : "dark")} className="grid h-9 w-9 place-items-center rounded-full border border-transparent text-slate-500 transition hover:border-slate-200 hover:bg-white/70 dark:text-slate-400 dark:hover:border-white/10 dark:hover:bg-white/10" aria-label="Toggle color theme"><Sun className="hidden h-[18px] w-[18px] dark:block" /><Moon className="h-[18px] w-[18px] dark:hidden" /></button>
            ...
            <Button onClick={start} className="landing-primary-button h-10 rounded-full px-4 text-sm font-semibold text-white sm:px-5"><span className="hidden sm:inline">{isAuthenticated ? "Open dashboard" : "Start for free"}</span><span className="sm:hidden">Start</span><ArrowUpRight className="ml-1.5 h-4 w-4" /></Button>
        ...
        {menuOpen && <nav className="landing-nav-glass mx-auto mt-2 max-w-[1180px] rounded-[20px] p-3 lg:hidden">{nav.map(([label, href]) => <a key={label} href={href} onClick={() => setMenuOpen(false)} className="block rounded-xl px-3 py-3 text-sm font-medium transition-colors hover:bg-white/60 dark:hover:bg-white/10">{label}</a>)}</nav>}
      </header>
      ...
      <main id="top">
        <section className="relative overflow-hidden pb-20 pt-36 sm:pb-24 sm:pt-44 lg:min-h-[820px] lg:pb-32 lg:pt-48">
          ...
          <div className="relative mx-auto grid max-w-[1240px] items-center gap-16 px-5 lg:grid-cols-[0.88fr_1.12fr] lg:gap-14 lg:px-8">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              <div className="landing-pill mb-7 inline-flex items-center gap-2.5 rounded-full px-4 py-2 text-[10px] font-bold uppercase tracking-[0.16em] text-blue-700 dark:text-blue-300"><Sparkles className="h-3.5 w-3.5" /> AI meeting intelligence <span className="h-1 w-1 rounded-full bg-blue-400" /> Built for focus</div>
              <h1 className="max-w-2xl text-[3.45rem] font-semibold leading-[0.94] tracking-[-0.065em] sm:text-[4.7rem] lg:text-[5.15rem]">Turn every meeting into <span className="landing-text-gradient">momentum.</span></h1>
              <p className="mt-7 max-w-[35rem] text-[1.05rem] leading-[1.75] tracking-[-0.01em] text-slate-600 sm:text-lg dark:text-slate-300">NoteFlow captures the conversation, finds the signal, and turns every decision into accountable action&mdash;before momentum is lost.</p>
              <div className="mt-9 flex flex-col gap-3 sm:flex-row"><Button onClick={start} className="landing-primary-button h-13 rounded-full px-7 text-[15px] font-semibold text-white">{isAuthenticated ? "Open your workspace" : "Start your free workspace"}<ArrowRight className="ml-2 h-4 w-4" /></Button><a href="#product" className="landing-glass-button group inline-flex h-13 items-center justify-center rounded-full px-6 text-[15px] font-semibold text-slate-800 dark:text-white"><span className="mr-2 grid h-7 w-7 place-items-center rounded-full bg-slate-950 text-white transition-transform group-hover:scale-105 dark:bg-white dark:text-slate-950"><Play className="ml-0.5 h-3 w-3 fill-current" /></span>Watch the workflow</a></div>
        ...
        <section className="relative z-10 px-5"><div className="landing-logo-cloud mx-auto flex max-w-[1120px] -translate-y-7 flex-col items-center gap-7 rounded-[1.6rem] px-6 py-7 sm:flex-row sm:justify-between"><p className="text-center text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 sm:text-left">Trusted by teams building what&apos;s next</p><div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-4 sm:gap-x-10">{["Northstar", "Vertex", "Spherule", "Capsule", "Arc Labs"].map((name, i) => <span key={name} className="flex items-center gap-1.5 text-sm font-bold tracking-tight text-slate-400 transition-colors hover:text-slate-700 dark:text-slate-500 dark:hover:text-slate-200"><span className={`h-3.5 w-3.5 ${i % 2 ? "rotate-45 rounded-[3px]" : "rounded-full"} bg-gradient-to-br from-slate-300 to-slate-400 dark:from-slate-600 dark:to-slate-500`} />{name}</span>)}</div></div></section>

        <section className="relative py-20 sm:py-28" aria-labelledby="benefit-heading"><div className="landing-orb landing-orb-violet pointer-events-none absolute -left-64 top-1/3 h-[460px] w-[460px] rounded-full opacity-50" /><div className="mx-auto max-w-[1240px] px-5 lg:px-8"><motion.div {...fadeUp} className="mx-auto max-w-3xl text-center"><p className="mb-4 text-xs font-bold uppercase tracking-[0.2em] text-blue-600 dark:text-blue-400">From talk to traction</p><h2 id="benefit-heading" className="text-3xl font-semibold tracking-[-0.04em] sm:text-5xl">Everything after the meeting,<br className="hidden sm:block" /> beautifully handled.</h2><p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-slate-600 sm:text-lg dark:text-slate-300">One focused workspace to capture what was said, understand what matters, and make sure the work actually moves forward.</p></motion.div><div className="mt-14 grid gap-5 lg:grid-cols-3">{benefits.map((benefit, index) => <motion.article key={benefit.title} {...fadeUp} transition={{ duration: 0.5, delay: index * 0.08 }} className="landing-feature-card group overflow-hidden rounded-[1.75rem] p-6 transition-all duration-500 hover:-translate-y-2 sm:p-7"><div className="flex items-center justify-between"><span className={`grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br ${benefit.color} text-white shadow-lg shadow-blue-500/15 transition-transform duration-500 group-hover:rotate-3 group-hover:scale-105`}><benefit.icon className="h-5 w-5" /></span><span className="text-xs font-bold tracking-[0.18em] text-slate-300 dark:text-white/20">{benefit.number}</span></div><h3 className="mt-7 text-xl font-semibold tracking-[-0.025em]">{benefit.title}</h3><p className="mt-3 min-h-20 text-sm leading-6 text-slate-600 dark:text-slate-400">{benefit.text}</p><BenefitVisual type={benefit.visual} /></motion.article>)}</div></div></section>

        <section id="workflow" className="relative scroll-mt-24 px-5 py-10 text-white sm:py-16"><div className="landing-dark-panel relative mx-auto max-w-[1220px] overflow-hidden rounded-[2.25rem] px-6 py-16 sm:px-10 sm:py-20 lg:px-16"><div className="landing-grid pointer-events-none absolute inset-0 opacity-15" /><div className="pointer-events-none absolute -right-32 -top-32 h-96 w-96 rounded-full bg-violet-500/25 blur-[100px]" /><div className="pointer-events-none absolute -bottom-32 left-1/4 h-96 w-96 rounded-full bg-blue-500/20 blur-[110px]" /><div className="relative mx-auto grid max-w-[1080px] gap-12 lg:grid-cols-[0.72fr_1.28fr] lg:gap-20"><motion.div {...fadeUp}><p className="mb-4 text-xs font-bold uppercase tracking-[0.2em] text-blue-300">A simpler workflow</p><h2 className="text-3xl font-semibold leading-tight tracking-[-0.04em] sm:text-5xl">Clarity in.<br /><span className="text-blue-300">Momentum out.</span></h2><p className="mt-5 max-w-md text-base leading-7 text-slate-300">No complex setup, no manual formatting, and no chasing people after the call. Just a reliable path from conversation to action.</p></motion.div><motion.div {...fadeUp} className="space-y-3">{workflow.map(([title, text], index) => <div key={title} className="landing-workflow-row group grid grid-cols-[42px_1fr] gap-4 rounded-2xl p-5 transition-colors sm:grid-cols-[52px_150px_1fr] sm:items-center"><span className="grid h-10 w-10 place-items-center rounded-xl border border-white/15 bg-white/[0.06] text-xs font-semibold text-blue-200 transition group-hover:border-blue-300/40 group-hover:bg-blue-400/10">0{index + 1}</span><h3 className="text-base font-semibold sm:text-lg">{title}</h3><p className="col-start-2 text-sm leading-6 text-slate-400 sm:col-start-auto">{text}</p></div>)}</motion.div></div></div></section>

        <section id="results" className="relative scroll-mt-24 px-5 py-20 sm:py-28"><div className="landing-orb landing-orb-cyan pointer-events-none absolute -right-48 top-12 h-[480px] w-[480px] rounded-full opacity-60" /><motion.div {...fadeUp} className="landing-results-card relative mx-auto max-w-[1160px] overflow-hidden rounded-[2.25rem] text-white"><div className="pointer-events-none absolute inset-0 bg-[linear-gradient(120deg,rgba(255,255,255,0.12),transparent_42%)]" /><div className="relative grid lg:grid-cols-[1.15fr_0.85fr]"><div className="p-8 sm:p-12 lg:p-16"><div className="mb-7 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-blue-100"><Sparkles className="h-4 w-4 fill-blue-200 text-blue-200" /> Customer story</div><blockquote className="max-w-2xl text-2xl font-medium leading-[1.35] tracking-[-0.025em] sm:text-4xl">&ldquo;NoteFlow gave us back the first ten minutes of every workday. Everyone arrives informed, and every decision has an owner.&rdquo;</blockquote><div className="mt-8 flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-full border border-white/20 bg-white/15 text-sm font-bold backdrop-blur-xl">MK</div><div><p className="text-sm font-semibold">Maya Khan</p><p className="text-xs text-blue-100">VP of Product, Northstar</p></div></div></div><div className="grid grid-cols-2 border-t border-white/15 bg-white/[0.06] backdrop-blur-xl lg:grid-cols-1 lg:border-l lg:border-t-0"><Metric value="6.4 hrs" label="saved per person, every month" /><Metric value="91%" label="of action items completed on time" divider /><Metric value="2.3x" label="faster decision follow-through" divider className="col-span-2 lg:col-span-1" /></div></div></motion.div></section>

        <section id="security" className="scroll-mt-24 px-5 py-6"><motion.div {...fadeUp} className="landing-security-card mx-auto grid max-w-[1100px] gap-8 rounded-[1.75rem] p-7 sm:grid-cols-[auto_1fr] sm:items-center sm:p-9"><div className="grid h-16 w-16 place-items-center rounded-2xl border border-emerald-200/60 bg-emerald-50/80 text-emerald-600 shadow-[0_12px_30px_-16px_rgba(5,150,105,0.5)] dark:border-emerald-400/15 dark:bg-emerald-500/10"><ShieldCheck className="h-8 w-8" /></div><div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-center"><div><h2 className="text-2xl font-semibold tracking-[-0.03em]">Your conversations stay yours.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-400">Encrypted data, workspace permissions, and privacy-minded AI processing keep your team knowledge protected.</p></div><div className="flex flex-wrap gap-3 text-xs font-semibold text-slate-600 dark:text-slate-300">{["Encrypted", "Role-based access", "Audit ready"].map((item) => <span key={item} className="rounded-full border border-slate-200/80 bg-white/50 px-3 py-1.5 dark:border-white/10 dark:bg-white/5">{item}</span>)}</div></div></motion.div></section>

        <section className="px-5 py-20 sm:py-28"><motion.div {...fadeUp} className="landing-cta-panel relative mx-auto max-w-[1040px] overflow-hidden rounded-[2.25rem] px-6 py-16 text-center sm:px-10 sm:py-20"><div className="landing-grid pointer-events-none absolute inset-0 opacity-25" /><div className="pointer-events-none absolute left-1/2 top-0 h-72 w-[620px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-blue-400/30 blur-[90px]" /><div className="relative"><p className="mb-4 text-xs font-bold uppercase tracking-[0.2em] text-blue-200">Your next meeting can be different</p><h2 className="text-4xl font-semibold leading-tight tracking-[-0.045em] text-white sm:text-6xl">Less meeting debt.<br />More meaningful progress.</h2><p className="mx-auto mt-6 max-w-xl text-base leading-7 text-slate-300">Create your workspace today and turn the next conversation into a plan everyone understands.</p><Button onClick={start} className="mt-8 h-13 rounded-full bg-white px-7 text-[15px] font-semibold text-slate-950 shadow-xl shadow-blue-950/20 transition hover:bg-blue-50">{isAuthenticated ? "Go to your dashboard" : "Get started for free"}<ArrowRight className="ml-2 h-4 w-4" /></Button></div></motion.div></section>
      </main>

      <footer className="border-t border-slate-200/60 bg-white/35 backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.02]"><div className="mx-auto max-w-[1240px] px-5 py-10 lg:px-8"><div className="flex flex-col gap-8 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2.5"><Logo size={30} /><Wordmark className="text-sm" /></div><p className="mt-3 text-xs text-slate-500">Where conversations become progress.</p></div><div className="flex flex-wrap gap-x-6 gap-y-3 text-sm font-medium text-slate-500">{nav.slice(0, 3).map(([label, href]) => <a key={label} href={href} className="transition-colors hover:text-slate-950 dark:hover:text-white">{label}</a>)}<a href="#" className="transition-colors hover:text-slate-950 dark:hover:text-white">Privacy</a><a href="#" className="transition-colors hover:text-slate-950 dark:hover:text-white">Terms</a></div></div><div className="mt-8 flex flex-col gap-3 border-t border-slate-200/70 pt-6 text-xs text-slate-400 sm:flex-row sm:justify-between dark:border-white/10"><p>Copyright {new Date().getFullYear()} NoteFlow AI.</p><span className="flex items-center gap-2"><span className="relative flex h-2 w-2"><span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-50" /><span className="relative h-2 w-2 rounded-full bg-emerald-500" /></span> All systems operational</span></div></div></footer>
    </div>
  );
}
...
function InteractiveProductPreview() {
  ...
  return (
    ...
    >
      ...
      <motion.div animate={{ y: [0, -7, 0] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }} className="landing-floating-card absolute -bottom-6 -right-2 hidden w-48 rounded-2xl p-3 sm:block lg:-right-8">
