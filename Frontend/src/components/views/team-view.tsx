...
import {
  ...
  Crown, Search, Filter, Mail, Key, UserCheck, Sparkles, MoreHorizontal,
} from "lucide-react"
import { useAppDispatch, useAppSelector } from "@/lib/redux/hooks"
import { updateTeam, addTeamMember, updateTeamMember, removeTeamMember } from "@/lib/redux/dataSlice"
...
const ROLE_BADGES: Record<string, { label: string; style: string; bar: string; icon: typeof Shield }> = {
  OWNER: { label: "Owner", style: "bg-amber-500/15 text-amber-400 border-amber-500/30", bar: "bg-amber-500", icon: Crown },
  LEAD: { label: "Lead", style: "bg-indigo-500/15 text-indigo-400 border-indigo-500/30", bar: "bg-indigo-500", icon: Shield },
  MEMBER: { label: "Member", style: "bg-slate-500/15 text-slate-400 border-slate-500/30", bar: "bg-slate-400", icon: Users },
}
...
export function TeamView() {
  ...
  const leadCount = currentTeam?.members.filter((m) => m.role === "LEAD" || m.role === "OWNER").length || 0
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            {isEditingTeam ? (
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Button size="sm" variant="ghost" onClick={() => setIsEditingTeam(false)} className="h-7 rounded-lg px-2">
              ...
              <div className="min-w-0">
                ...
                <p className="truncate text-[11px] text-muted-foreground">
                  Workspace: <span className="font-semibold text-foreground">{activeWs.name}</span>
                </p>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="hidden sm:flex items-center gap-2 border-r border-white/10 pr-3 text-[11px]">
              <span className="rounded-lg bg-muted/40 px-2 py-1 font-semibold text-muted-foreground">
                👥 <strong className="text-foreground">{totalCount}</strong> Members
              </span>
              <span className="rounded-lg bg-muted/40 px-2 py-1 font-semibold text-muted-foreground">
            ...
            {(["ALL", "OWNER", "LEAD", "MEMBER"] as const).map((r) => (
              <button
                ...
                className={cn(
                  ...
                  roleFilter === r ? "bg-indigo-500 text-white font-bold" : "text-muted-foreground hover:text-foreground"
                )}
              >
        ...
        <div className="grid gap-2.5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {filteredMembers.map((member, idx) => {
            ...
            return (
              ...
              >
                <div
                  className="group relative flex items-center justify-between overflow-hidden rounded-xl border border-white/10 bg-gradient-to-br from-card/90 via-card/60 to-background/50 p-2.5 shadow-sm backdrop-blur-xl transition-all duration-300 hover:border-indigo-500/40 hover:bg-card/95 hover:shadow-lg hover:shadow-indigo-500/10 hover:-translate-y-0.5"
                >
                  {/* Subtle top role accent bar */}
                  <div className={cn("absolute top-0 left-0 right-0 h-[2px] opacity-75 transition-opacity group-hover:opacity-100", RoleMeta.bar)} />
                  ...
                  <div className="flex items-center gap-2.5 min-w-0 pr-1">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex items-center gap-1">
                        <Badge variant="outline" className={cn("gap-0.5 px-1.5 py-0 text-[8px] font-bold leading-none shrink-0 shadow-inner", RoleMeta.style)}>
            ...
            >
              <div className="flex items-center justify-between border-b border-border/40 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground">Update member details and role</p>
                  </div>
                </div>
                <button
                  onClick={() => setEditingMember(null)}
              ...
              <form onSubmit={handleSaveMemberEdit} className="mt-3.5 space-y-3">
                ...
                <div className="flex items-center justify-end gap-2.5 pt-2">
                  <Button type="button" variant="outline" size="sm" onClick={() => setEditingMember(null)} className="rounded-lg h-8 text-xs">
                    Cancel
                  </Button>
                  <Button type="submit" size="sm" className="gap-1.5 rounded-lg h-8 bg-indigo-500 text-xs text-white shadow-md">
