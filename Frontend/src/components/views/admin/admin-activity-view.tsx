      ...
      >
        <Card className="border-border/60 bg-card/70 p-6 backdrop-blur-sm">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {[
              { title: "Login History", desc: "Track all user login attempts with IP, device, and browser info" },
              { title: "Role Changes", desc: "Log all role assignments and permission modifications" },
              { title: "Account Events", desc: "User creation, deletion, suspension, and reactivation events" },
              { title: "Security Alerts", desc: "Failed login attempts, suspicious activity, and rate limiting" },
            ].map((item) => (
              <div
                key={item.title}
