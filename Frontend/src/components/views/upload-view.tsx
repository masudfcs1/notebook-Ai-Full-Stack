      ...
      const interval = setInterval(() => {
        ...
        if (progress >= 100) {
          ...
          setFiles((prev) =>
            prev.map((f) =>
              f.id === id
                ? { ...f, progress: 100, status: "done", content: text }
                : f,
            ),
          );
            ...
            <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
              {ACCEPTED.map((ext) => (
                <Badge
                  key={ext}
                  variant="secondary"
                  className="bg-muted/60 text-[11px] uppercase"
                >
                  {ext}
                </Badge>
              ))}
            ...
            {files.length > 0 && (
              ...
              >
                ...
                {files.map((f) => (
                  <Card
                    key={f.id}
                    className="flex items-center gap-3 border-border/50 p-4 backdrop-blur-sm"
                  >
                    <div
                      className={cn(
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <Badge variant="outline" className="text-[10px]">
                  {manualContent.trim()
                    ? manualContent.trim().split(/\s+/).length
                    : 0}{" "}
                  words
                </Badge>
