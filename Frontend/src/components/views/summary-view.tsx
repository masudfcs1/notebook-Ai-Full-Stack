...
export function SummaryView() {
  ...
  const initialId =
    activeNoteId && summarizedNotes.some((n) => n.id === activeNoteId)
      ? activeNoteId
  ...
  const noteTaskCount = useAppSelector(
    (s) => s.data.tasks.filter((t) => t.noteId === note?.id).length,
  );
                ...
                <SelectItem key={n.id} value={n.id}>
                  <span className="flex items-center gap-2">
                    ...
                    {n.status === "summarized" && (
                      <Badge
                        variant="secondary"
                        className="ml-1 h-4 bg-emerald-500/10 px-1 text-[9px] text-emerald-600"
                      >
                        ✓
                      </Badge>
                    )}
                ...
                {generating ? (
                  ...
                  >
                    <div className="flex gap-1.5">
                      <span
                        className="ai-dot h-2.5 w-2.5 rounded-full bg-indigo-500"
                        style={{ animationDelay: "0ms" }}
                      />
                      <span
                        className="ai-dot h-2.5 w-2.5 rounded-full bg-violet-500"
                        style={{ animationDelay: "200ms" }}
                      />
                      <span
                        className="ai-dot h-2.5 w-2.5 rounded-full bg-fuchsia-500"
                    ...
                    <p className="text-[11px] text-muted-foreground/70">
                      Extracting key points, decisions & tasks
                    </p>
                  </motion.div>
                ) : summary ? (
                  <motion.div
  ...
  return (
    <div className="flex items-start gap-2.5 rounded-xl bg-background/40 p-3">
      ...
      <div className="flex-1">
        ...
        <button
          onClick={action}
          className="mt-1.5 text-[11px] font-medium text-indigo-500 hover:underline"
        >
          {actionLabel} →
        </button>
      </div>
