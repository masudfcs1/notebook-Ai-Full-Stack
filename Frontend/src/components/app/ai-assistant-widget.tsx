  ...
  async function send(text: string) {
    ...
    try {
      ...
      setMessages([
        ...
        {
          role: "assistant",
          content: "I hit a snag reaching the AI service. Please try again.",
        },
      ]);
    } finally {
      setLoading(false);
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          >
            {/* Header */}
            <div className="flex items-center gap-3 border-b border-border/40 bg-linear-to-r from-indigo-500/10 to-violet-500/10 px-4 py-3">
            ...
            <div
              ref={scrollRef}
              className="flex-1 space-y-3 overflow-y-auto scrollbar-thin px-3 py-4"
            >
              {messages.map((m, i) => (
                <div
                  key={i}
              ...
              {loading && (
                <div className="flex gap-2">
                  ...
                  <div className="flex items-center gap-1 rounded-2xl rounded-tl-sm bg-muted/60 px-4 py-3">
                    <span
                      className="ai-dot h-2 w-2 rounded-full bg-indigo-500"
